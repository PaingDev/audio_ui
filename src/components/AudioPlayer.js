...
export default function AudioPlayer({ track, style }) {
    ...
    <>
      <AudioPlayerContext.Provider value={{ analyser, playing }}>
        <div className="fixed bottom-0 pt-3 px-5 w-full bg-stone-800 text-center shadow-lg">
          ...
          <input
            style={{
              background: `linear-gradient(90deg, #777 ${(seek / duration) *
                100}%, white ${(seek / duration) * 100}%)`
            }}
